...
namespace TacticalAI
{
    public class Idle : TacticalAI.CustomAIBehaviour
    {
        public override void Initiate()
        {
            base.Initiate();
...
namespace TacticalAI
{
    public class Skirmish : TacticalAI.CustomAIBehaviour
    {
        ...
        public AttackPositions attackPos;
        public override void Initiate()
        {
            base.Initiate();
        ...
        public override void AICycle()
        {
            ...
            if (timeLeftAtThisPoint <= 0)
            {
                timeLeftAtThisPoint = maxTimeToWaitAtEachPoint * Random.value;
                baseScript.isReached = false;

                targetVector = GetNewDestTarget(baseScript.targetTransform);
            }
            //Check if we have arrived at the new targetvector
            else 
            if (haveADestTarget && framesUntilCanReachObject < 0 && navI.ReachedDestination() && !baseScript.usingDynamicObject)
            {
                Debug.Log("Skirmish isReached");
        ...
        Vector3 GetNewDestTarget(Transform targ)
        {
            ...
            if (baseScript.myIdleBehaviour == BaseScript.IdleBehaviour.MoveToTransform  && ControllerScript.currentController.positionsList.Count > 0)
            {
                AttackPositions tempAtt = ControllerScript.currentController.GetAttackPosition();
                if (attackPos)
                {
                    ControllerScript.currentController.ReturnAttackPostion(attackPos);
                }
                attackPos = tempAtt;
                baseScript.isReached = tempAtt.isCrouchPosition;
                return attackPos.transform.position;
            }
            else
           // Debug.Log("AI "+ baseScript.isReached);
            if (!returnPos.Equals(Vector3.zero))
            {
                baseScript.isReached = Random.Range(0, 100) > 50 ? true : false;
