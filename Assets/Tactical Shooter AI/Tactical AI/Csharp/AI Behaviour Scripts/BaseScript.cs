...
namespace TacticalAI
{
    public class BaseScript : MonoBehaviour
    {
        ...
        public enum AIType
        {
            Berserker = 0, Tactical = 1, Custom = 2, Skirmish = 3,Idle,
        }

        public enum IdleBehaviour
        {
            Search = 1, Patrol = 2, Wander = 3, MoveToTransform = 4, Custom = 5,Idle=6,
        }

        public enum AIBehaviour
        {
            GoToMoveTarget = 0, Cover = 1, Search = 2, Dodging = 3, Patrolling = 4, Wander = 5, UseDynamicObject = 6, InvestigateSound = 7, Custom = 8, MoveToTransform = 9, RunFromGrenade = 10, Skirmish = 11, Parkour = 12, Stagger = 13, ThrowGrenade = 14,Idle=15
        }
        ...
        void Start()
        {
            ...
            hUDNavigationElement = gameObject.GetComponentInChildren<HUDNavigationElement>();
        }
            ...
            while (this.enabled)
            {
                ...
                if (!isDodging)
                {
                    ...
                    else if(currentBehaviour)
                    {
                        ...
                        else if (!isMeleeing && myIdleBehaviour== IdleBehaviour.MoveToTransform&& Vector3.SqrMagnitude(myTransform.position - currentBehaviour.targetVector) > 10 && canSprint)
                        {
                            isSprintingForcefully = true;
                            animationScript.StartSprinting();
                            SetSprintSpeed();
                            headLookScript.Deactivate();
                        }
                        else if(isSprintingForcefully && Vector3.SqrMagnitude(myTransform.position - currentBehaviour.targetVector)<5 )
                        {
                            isSprintingForcefully = false;
                            isSprinting = false;
                            animationScript.StopSprinting();
                            SetProperSpeed();
                            myTargetScript.StartAttack();
                        }
                    }
                }
                MoveAI();
        ...
        CustomAIBehaviour GetIdleBehaviour()
        {
            switch (myIdleBehaviour)
            {
                ...
                case IdleBehaviour.Idle: return GetNewBehaviour(AIBehaviour.Idle);
        ...
        public CustomAIBehaviour GetNewBehaviour(AIBehaviour t)
        {
            ...
            switch (t)
            {
                ...
                case AIBehaviour.Idle: bToReturn = (TacticalAI.CustomAIBehaviour)gameObject.AddComponent(typeof(TacticalAI.Idle)); break;
        ...
        {
            if (myAIType == BaseScript.AIType.Idle)
            {
                return;
            }
        ...
        public void KillAI()
        {
            ...
            if (this.enabled)
            {
                ...
                if (cWave)
                {
                    cWave.RemoveEnemy(this.gameObject);
                }
            }
        }
