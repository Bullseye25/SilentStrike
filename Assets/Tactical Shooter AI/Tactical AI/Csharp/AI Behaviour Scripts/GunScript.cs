...
public class GunScript : MonoBehaviour {
    ...
    public float maximumDistToFireGun = 9999;
		...
		Vector3 offsectVector = new Vector3(0, 0.5f, 0);
	...
	void Start () {
		enemyTeams = myAIBaseScript.GetEnemyTeamIDs();
			bulletParticle = bulletSpawn.GetComponentInChildren<ParticleSystem>();
	...
	void FireOneShot()
	{
		...
		if(targetTransform && !myAIBaseScript.inParkour)
			{	
			    ...
			    bool amAtTarget = Vector3.Angle(bulletSpawn.forward, (targetTransform.position - offsectVector) - bulletSpawn.position) < maxFiringAngle;
				...
				for(int j = 0; j < pelletsPerShot; j++)
                    ...
                    if (amAtTarget)
                    {
                        fireRotation.SetLookRotation((targetTransform.position- offsectVector) - bulletSpawn.position);
                    }
                    else
                    {
					...
					bulletParticle.transform.rotation = fireRotation;
					//GameObject bullet = (GameObject)(Instantiate(bulletObject, bulletSpawn.position, fireRotation));
					GameObject bullet = AzuObjectPool.instance.SpawnPooledObj(21, bulletSpawn.position, fireRotation) as GameObject;
				...
				if(muzzleFlash)
					...
					//muzzleFlash.transform.parent = muzzleFlashSpawn;
