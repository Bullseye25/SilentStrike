...
public class bl_UCrosshair : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            int id = currentCross + 1;
            id = id % Crosshairs.Length;
            Debug.Log("id " + id);
    ...
    void RaycastControll()
    {
        ...
        if (Physics.Raycast(ray.origin, ray.direction, out hit, TargetDetectMaxDistance, TargetLayer))
        {
            if (hit.transform.gameObject.layer == 13 || hit.transform.gameObject.layer == 9)
            {
                GetCrosshair.SetColor(OnTargetFocusColor, ColorTransitionDuration);
                if (isaming)
                {
                    Targetai = hit.transform.root;
                }
            }
        }
        else
    ...
    float lastTimeTargetCheck;
    void AutoAssist()
    {
        if (!useDetection || RenderCamera == null)
            return;
        if (Time.time< lastTimeTargetCheck+5)
        {
            return;
        }
        Ray ray = RenderCamera.ViewportPointToRay(new Vector3(0.5f, 0.5f, 0));
        ...
        //Vector3 point = ray.origin + (ray.direction * 100);
        if (Physics.SphereCast(ray.origin,0.5f ,ray.direction,out raycastHit, TargetDetectMaxDistance, TargetLayer))
        {
           // HudMenuManager.instance.player.MouseLookComponent.Setplayerpostion_rotation(raycastHit.transform);
           // Debug.Log("raycastHit.transform "+ raycastHit.transform.name);
            lastTimeTargetCheck = Time.time;
          // GetCrosshair.SetColor(OnTargetFocusColor, ColorTransitionDuration);
