...
public class WeaponPickup : MonoBehaviour {
	...
	public void PickUpItem (){
		...
		if(!WeaponBehaviorComponent.haveWeapon){
            if (PlayerWeaponsComponent.currentWeapon == PlayerWeaponsComponent.Gun1 + 1)
            {
                PlayerWeaponsComponent.Gun1 = this.weaponNumber-1;
            }
            else
            {
                PlayerWeaponsComponent.Gun2 = this.weaponNumber-1;
            }
            // Debug.LogError("if player does not have this weapon, pick it up");
            ...
            if (PlayerWeaponsComponent.totalWeapons == PlayerWeaponsComponent.maxWeapons && WeaponBehaviorComponent.addsToTotalWeaps){
				...
				if(CurrentWeaponBehaviorComponent.droppable){
					...
					if(removeOnUse && !WeaponBehaviorComponent.dropWillDupe && !CurrentWeaponBehaviorComponent.dropWillDupe){
                      ...
                      //  Debug.LogError("drop current weapon if dropCurrentWeapon is true");
                    }
                       ...
                       // Debug.LogError("replace current weapon if dropCurrentWeapon is false");
                    }
					
				}else{//currently held weapon is not dropable, so find next item in the weaponOrder array that is droppable, and drop it
                   // Debug.LogError("urrently held weapon is not dropable, so find next item in the weaponOrder array that is droppable");
