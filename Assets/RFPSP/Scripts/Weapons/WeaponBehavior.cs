...
public class WeaponBehavior : MonoBehaviour {
	...
	public autoMoveAndRotate WeaponRotationScript;
	...
	[Tooltip("FOV value to use when Not zoomed, lower values with scoped weapons for more zoom.")]
	public float weaponCamFovDiff = 0;
    ...
    private void OnEnable()
    {
        ...
        {
			FPSPlayer.instance.IronsightsComponent.weaponCamFovDiff = weaponCamFovDiff;

		}
	}
    void Start (){
        instance = this;
        MConstants.Tempweapon = this.gameObject;
		...
		if(WeaponRotationScript)
        {
			WeaponRotationScript.enabled = false;
		}
    ...
    void LateUpdate(){
		...
		if (Time.timeScale > 0.0f && Time.deltaTime > 0.0f && Time.smoothDeltaTime > 0.0f){//allow pausing by setting timescale to 0
            ...
            {
                RaycastHit hit2;
                if (Physics.Raycast(mainCamTransform.position, mainCamTransform.forward, out hit2, range))
                {
					//if(hit2.transform.gameObject.layer == 13 && hit2.collider.gameObject.GetComponent<TacticalAI.HitBox>())
     //               {
					//	autoFire = true;
     //               }
                    if (hit2.transform.gameObject.layer == 17 || hit2.transform.gameObject.layer == 15 || hit2.transform.gameObject.layer == 0)
                    {
	                    if (hit2.collider.gameObject.GetComponentInParent<EnemyAI>())
	                    {
		                    autoFire = !hit2.collider.gameObject.GetComponentInParent<EnemyAI>().killed;
	                    }
	                    else if (hit2.collider.gameObject.GetComponent<TacticalAI.HitBox>())
	                    {
		                    autoFire = true;
	                    }
	                    else if (hit2.transform.gameObject.layer == 0 && !hit2.collider.gameObject.GetComponent<TacticalAI.HitBox>())
	                    {
		                    autoFire = false;
	                    }
	                    else
                    ...
                    {
						if (hit2.transform.gameObject.layer == 13)
						{
							if (hit2.collider.gameObject.GetComponent<TacticalAI.HitBox>() && !hit2.collider.gameObject.GetComponent<TacticalAI.HitBox>().myScript.isdead
								|| hit2.collider.gameObject.GetComponent<DamageTaker>())
							{
								autoFire = true;
							}
						}
	...
	void Update(){
		...
		if (ControlFreak2.CF2Input.GetAxisRaw("Xbox R Trigger") > 0.1f || ControlFreak2.CF2Input.GetButtonDown("Fire"))
		{
			if (WeaponRotationScript)
			{
				WeaponRotationScript.enabled = true;
			}
		}
		if (ControlFreak2.CF2Input.GetAxisRaw("Xbox R Trigger") > 0.1f || ControlFreak2.CF2Input.GetButtonUp("Fire"))
		{
			if (WeaponRotationScript)
			{
		...
		if(Time.timeScale > 0){//allow pausing by setting timescale to 0
			...
			if(!unarmed && FPSPlayerComponent.hitPoints > 0){
                ...
                if (ammo <= 0 && bulletsLeft <= 0 && !MConstants.bulletsFinished)
                {
                    MConstants.bulletsFinished = true;
                    PlayerWeaponsComponent.CheckAllWeaponsAmmo();
	...
	private void Fire(){
		...
		if(!meleeActive &&
		   ((bulletsLeft <= 0 && doReload) 
		 || (!doReload && ammo <= 0)
		 || (semiAuto && semiState))||MConstants.IslastBullet ||MConstants.isPlayerWin)
        {
			return;
		}
			...
			if(projectilePoolIndex != 0 && !meleeActive){
				...
				if (!CameraControlComponent.thirdPersonActive)
				{
					...
					//{
						...
						projectile = AzuObjectPool.instance.SpawnPooledObj(projectilePoolIndex, muzzleParticles.transform.position, muzzleParticles.transform.rotation) as GameObject;

						if (Vector3.Distance(muzzleParticles.transform.position, tpRayHit.point) > 7 && tpRayHit.point != Vector3.zero)
						{
							projectile.transform.LookAt(tpRayHit.point);
				...
				if(meleeSwingDelay == 0 && !meleeActive){
					...
					if (useTracers){
						StartCoroutine(EmitTracers(direction , hit));
					}
	...
	void HitObject ( RaycastHit hit, Vector3 directionArg, bool isSecondCast = false ){
		...
		MConstants.BulletInfo = hit.point;
		MConstants.BulletFired = true;
		...
		case 0://hit object
			...
			if(hit.collider.gameObject.GetComponent<BreakableObject>()){
				...
				FPSPlayerComponent.UpdateHitTime();//used for hitmarker
			}else if (hit.collider.gameObject.GetComponent<ExplosiveDrum>()) {
				hit.collider.gameObject.GetComponent<ExplosiveDrum>().Explode();
			}
			else if (hit.collider.gameObject.GetComponent<TacticalAI.HitBox>())
			{
				hit.collider.gameObject.GetComponent<TacticalAI.HitBox>().Damage(damageAmt, 1, directionArg);
				GameObject blood = AzuObjectPool.instance.SpawnPooledObj(20, hit.transform.position, hit.transform.rotation);
				StartCoroutine(RecyclePooledBlood(blood));
				FPSPlayerComponent.UpdateHitTime();//used for hitmarker
			}
			break;
		case 1://hit object is an object with transparent effects like a window
			...
			break;
		case 9://hit object is an object with transparent effects like a window
                if (hit.collider.gameObject.GetComponent<JeepCTRL>())
                {
                    hit.collider.gameObject.GetComponent<JeepCTRL>().Stop_Jeep_And_Enable_AI();
		...
		case 13://hit object is an NPC
				...
				{
					hit.collider.gameObject.GetComponent<TacticalAI.HitBox>().Damage(damageAmt, directionArg);
					FPSPlayerComponent.UpdateHitTime();//used for hitmarker
				}

				if (hit.collider.gameObject.GetComponent<DamageTaker>())
				{

					hit.collider.gameObject.GetComponent<DamageTaker>().OnHit(damageAmt);
					//FPSPlayerComponent.UpdateHitTime();//used for hitmarker
				}
				...
				if (hit.collider.gameObject.GetComponent<CharacterDamage>() && hit.collider.gameObject.GetComponent<AI>().enabled)
				{
					...
					FPSPlayerComponent.UpdateHitTime();//used for hitmarker
					Debug.Log("ENEMY GOT HIT BY PLAYER");
				}


				if (hit.collider.gameObject.GetComponent<LocationDamage>() && hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.enabled && !MConstants.IslastBullet)
            {
	            if (hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.gameObject.GetComponent<CharacterDamage>().hitPoints - damageAmt <= 0 
	                && hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.gameObject.GetComponent<CharacterDamage>().hitPoints > 0 
	                && HudMenuManager.instance.RegisteredEnemyToKill == 1 && MConstants.CurrentGameMode != MConstants.GAME_MODES.SURVIVAL_MODE)
	            {
		            MConstants.IslastBullet = true;
		            hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.gameObject.GetComponent<NPCAttack>().damage = 0f;
		            MConstants.LastBulletTarget = hit.collider.gameObject.GetComponent<LocationDamage>().gameObject;
		            FPSPlayer.instance.painFadeObj.GetComponent<Image>().enabled = false;
		            HudMenuManager.instance.FpsCamera.farClipPlane = 1f;
		            Instantiate(HudMenuManager.instance.LastBulletPrefab, mainCamTransform.transform.position, mainCamTransform.transform.rotation);
		            //this.ammo = 0;
		            //this.bulletsLeft = 0;
		            UnityEngine.Debug.Log("last Bullet Effect");
	            }
	            else
	            {
		            if (hit.collider.gameObject.GetComponent<LocationDamage>().damageMultiplier > 15f 
		                && hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.gameObject.GetComponent<CharacterDamage>().hitPoints > 0 
		                && HudMenuManager.instance.RegisteredEnemyToKill == 1 && MConstants.CurrentGameMode != MConstants.GAME_MODES.SURVIVAL_MODE)
		            {
			            MConstants.IslastBullet = true;
            ...
            if (hit.collider.gameObject.GetComponent<HeliCharatersCtrl>())
            {
	            hit.collider.gameObject.GetComponent<HeliCharatersCtrl>().ApplyDamage(damageAmt);
	            //FPSPlayerComponent.UpdateHitTime();//used for hitmarker
            }
            if (hit.collider.gameObject.GetComponent<SocideBoomber>() && !MConstants.IslastBullet)
            {
	            if (hit.collider.gameObject.GetComponent<SocideBoomber>().ShootCount-damageAmt <= 0 && HudMenuManager.instance.RegisteredEnemyToKill == 1 && MConstants.CurrentGameMode != MConstants.GAME_MODES.SURVIVAL_MODE)
	            {
		            MConstants.IslastBullet = true;
                       
		            MConstants.LastBulletTarget = hit.collider.gameObject.GetComponent<SocideBoomber>().gameObject;
		            FPSPlayer.instance.painFadeObj.GetComponent<Image>().enabled = false;
		            HudMenuManager.instance.FpsCamera.farClipPlane = 1f;
		            Instantiate(HudMenuManager.instance.LastBulletPrefab, mainCamTransform.transform.position, mainCamTransform.transform.rotation);
                     
		            UnityEngine.Debug.Log("sucideboobmber");
	            }
	            else
	            {

		            hit.collider.gameObject.GetComponent<SocideBoomber>().DamageApplie(damageAmt);
	            }
	            //FPSPlayerComponent.UpdateHitTime();//used for hitmarker
            }

            if (hit.collider.gameObject.GetComponent<DestroableHelicopterCtRL>())
            {
                 
	            hit.collider.gameObject.GetComponent<DestroableHelicopterCtRL>().HelicoperGotShotted(hit.point,damageAmt);
	            //FPSPlayerComponent.UpdateHitTime();//used for hitmarker
            }
            if (hit.collider.gameObject.GetComponentInParent<MachineGunCtrl>()&& !MConstants.IslastBullet)
            {
	            if (hit.collider.gameObject.GetComponentInParent<MachineGunCtrl>().health - damageAmt <= 0 && hit.collider.gameObject.GetComponentInParent<MachineGunCtrl>().health > 0 && HudMenuManager.instance.RegisteredEnemyToKill == 1 && MConstants.CurrentGameMode != MConstants.GAME_MODES.SURVIVAL_MODE)
	            {
		            MConstants.IslastBullet = true;
		            hit.collider.gameObject.GetComponentInParent<MachineGunCtrl>().FOVDetection.PlayerDamage = 0f;
		            MConstants.LastBulletTarget = hit.collider.gameObject.GetComponentInParent<MachineGunCtrl>().FOVDetection.transform.GetChild(0).transform.gameObject;
		            FPSPlayer.instance.painFadeObj.GetComponent<Image>().enabled = false;
		            HudMenuManager.instance.FpsCamera.farClipPlane = 1f;
		            Instantiate(HudMenuManager.instance.LastBulletPrefab, mainCamTransform.transform.position, mainCamTransform.transform.rotation);
		            //this.ammo = 0;
	            ...
	            {
		            hit.collider.gameObject.GetComponentInParent<MachineGunCtrl>().ApplyDamage(damageAmt);
	            }
            }
            break;
		case 15://hit object is an Enemy
			if (hit.collider.gameObject.GetComponentInParent<EnemyAI>() && !hit.collider.gameObject.GetComponentInParent<EnemyAI>().killed)
			{
				EnemyAI enemyAI = hit.collider.gameObject.GetComponentInParent<EnemyAI>();
				if (HudMenuManager.instance.RegisteredEnemyToKill == 1 && (enemyAI.health - damage) <= 0)
				{
					if (MConstants.CurrentGameMode == MConstants.GAME_MODES.MODE3_Zombie
					&& LevelsManager.instance.currentLevel.isMachineGunLevel)
					{
						HudMenuManager.instance.SlowMotionLastBullet();
						
						InstantiateBloodFX(hit);

						if ((enemyAI.health - damage) <= 0)
						{
					...
					{
						MConstants.IslastBullet = true;
						MConstants.LastBulletTarget = hit.collider.gameObject;
						MConstants.LastBulletHit = hit;
						FPSPlayer.instance.painFadeObj.GetComponent<Image>().enabled = false;
						HudMenuManager.instance.FpsCamera.farClipPlane = 1f;
                            HudMenuManager.instance.EnableAllUi(false);
                            HudMenuManager.instance.pauseBtn.SetActive(false);
						HudMenuManager.instance.disableMissionBg.SetActive(false);
							HudMenuManager.instance.missionTextBg.SetActive(false);
							Instantiate(HudMenuManager.instance.LastBulletPrefab, mainCamTransform.transform.position, mainCamTransform.transform.rotation);
						enemyAI.WaitForGunShot();
						if (hit.collider.CompareTag("head"))
						{
							MConstants.IsLastBulletHeadShot = true;
						}
					}
				}
				else
				{
					if ((enemyAI.health - damage) <= 0)
					{
						if(MConstants.CurrentGameMode == MConstants.GAME_MODES.MODE3_Zombie) InstantiateBloodFX(hit);
			...
			else if (hit.collider.gameObject.GetComponentInParent<Level33LadderEnemy>())
			{ 
				/*MConstants.IslastBullet = true;
					MConstants.LastBulletTarget = hit.collider.gameObject;
					MConstants.LastBulletHit = hit;
					...
					HudMenuManager.instance.pauseBtn.SetActive(false);
					Instantiate(HudMenuManager.instance.LastBulletPrefab, mainCamTransform.transform.position, mainCamTransform.transform.rotation);
					hit.collider.gameObject.GetComponentInParent<Level33LadderEnemy>().WaitForGunShot();
					if (hit.collider.CompareTag("head"))
					{
						MConstants.IsLastBulletHeadShot = true;
					}*/
				hit.collider.gameObject.GetComponentInParent<Level33LadderEnemy>().TakeDamage(damage);
				if (hit.collider.CompareTag("head"))
				{
					HudMenuManager.instance.HeadShotAlert(); 
					HudMenuManager.instance.headShotCount++;
			...
			break;
		case 16://hit object is a Civilian
			if (hit.collider.gameObject.GetComponentInParent<CivilianAI>())
			{
				hit.collider.gameObject.GetComponentInParent<CivilianAI>().CivilianDeath();
			}
			break;
		case 17://hit object is a Car or Helicopter
			if (hit.collider.gameObject.GetComponentInParent<CarDestroyer>())
			{
				hit.collider.gameObject.GetComponentInParent<CarDestroyer>().DestroyThisCar();
			}

			if (hit.collider.gameObject.GetComponent<HelicopterDestroyer>())
			{
				hit.collider.gameObject.GetComponent<HelicopterDestroyer>().DestroyHelicopter();
			}

			if (hit.collider.gameObject.GetComponent<CraneDestroyer>())
			{
				hit.collider.gameObject.GetComponent<CraneDestroyer>().DropCrane();
			}
			break;
		}
	...
	void InstantiateBloodFX(RaycastHit _hit)
	{
		var direction = _hit.normal;
		float angle = Mathf.Atan2(direction.x, direction.z) * Mathf.Rad2Deg + 180;
		var bloodFX = Instantiate(HudMenuManager.instance.bloodExplodeFx, _hit.point, Quaternion.Euler(0, angle + 90, 0));
		bloodFX.transform.SetParent(null);
		Destroy(bloodFX, 10f);
	}

	IEnumerator RecyclePooledBlood(GameObject go)
	{
		yield return new WaitForSeconds(0.5f);
		AzuObjectPool.instance.RecyclePooledObj(20, go);
	}

	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//Weapon Tracers
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	IEnumerator EmitTracers (Vector3 tracerDirection , RaycastHit hit)
	{
		...
		if (TraceParticles)
		{
			//Vector3 wPos = transform.TransformPoint(hit.point);

			if (Vector3.Distance(TraceParticles.transform.position, hit.point) > 7 && hit.point != Vector3.zero)
			{
				TraceParticles.transform.LookAt(hit.point);
		...
		if (muzzleFlash){
			...
			if(!FPSWalkerComponent.holdingBreath){
				if(muzzleLightObj && !CameraControlComponent.thirdPersonActive){
					...
					muzzleLightComponent.intensity = 4.0f;
				}
