...
public class PlayerCharacter : MonoBehaviour {
	...
	void Start () {
		...
		{
			LevelsManager.instance.fpBody = fpBodyObj.GetComponentInChildren<PlayerCharacterClone>().gameObject; //extra line added
			
			if (MConstants.CurrentGameMode == MConstants.GAME_MODES.MODE5_BATTLEFIELD)
				fpBodyObj.GetComponentInChildren<PlayerCharacterClone>().SetTacticalAiTargetTransform();
		}
			
		// else if(MConstants.CurrentGameMode == MConstants.GAME_MODES.SURVIVAL_MODE)
			// SurvivalLevelsManager.instance.fpBody = fpBodyObj.GetComponentInChildren<PlayerCharacterClone>().gameObject; //extra line added
