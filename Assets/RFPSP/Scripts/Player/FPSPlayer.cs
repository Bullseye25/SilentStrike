	...
	void Update (){
		...
		if(usePlayerHunger){
			...
			&& hitPoints > 0.0f){
				...
				if (hitPoints < 1.0f){
                    PlayerWeapons.instance.gameObject.SetActive(false);
		...
		bl_UCrosshair.Instance.OnAim(InputComponent.zoomHold);
			...
			if(!zoomStartState){
				...
				if(zoomEnd - zoomStart < zoomDelay * Time.timeScale){//if button is tapped, toggle zoom state
					if(!zoomed){
                        ...
                        zoomed = true;
                        foreach (var item in HudMenuManager.instance.Snipers)
                        {
                            if (item.activeSelf)
                            {
                        ...
                        //if (LevelsManager.instance.Levels[MConstants.CurrentLevelNumber - 1].isMachineGunLevel && HudMenuManager.instance.machineGunCrosshair)
                        //{
                        ...
                        //if (LevelsManager.instance.Levels[MConstants.CurrentLevelNumber - 1].isMachineGunLevel && MConstants.StartTimer && HudMenuManager.instance.machineGunCrosshair)
                        //{
		...
		&& !CameraControlComponent.thirdPersonActive){
			...
			|| WeaponBehaviorComponent.bulletsPerClip == WeaponBehaviorComponent.bulletsToReload){
				...
				if(zoomStopTime + 0.2f < Time.time){
					if(!crosshairVisibleState && MConstants.CurrentGameMode != MConstants.GAME_MODES.MODE5_BATTLEFIELD)
					{
     ...
     //                       if (1.8f > Vector3.Distance(hit.collider.gameObject.GetComponent<WeaponPickup>().gameObject.transform.position, mainCamTransform.transform.position) && playerDaed == false&& Time.time>=picktime+2f)
     //                       {
     //                           picktime = Time.time;
     ...
     //                           Debug.Log("player has maximum weapons");
     ...
     //                           {
     //                               //  Debug.LogError("//weapon adds to total inventory");
     //                               if (!useSwapReticle)
     ...
     //                                   WeaponPickupComponent.PickUpItem();
     //                                   Debug.Log("Weopon_Pickup");
     //                                   //useable
     //                               }
     ...
     //                                       pickupTex = pickupReticle;
     //                                       WeaponPickupComponent.PickUpItem();
     //                                       Debug.Log("Player Have allready Weapon");
     ...
     //                   Debug.Log("Enemy Health_PickUp");


     //                   HealthPickupComponent.PickUpItem(this.gameObject);
     ...
     //               }
     //               else
                        if (hit.collider.gameObject.GetComponent<AmmoPickup>())
                        {
                            //set up external script references
...
//	}
	...
	public void StartSlowMoTimeCoroutine()
    {
        StartCoroutine(ActivateBulletTime(0.9f));
    }
			...
			if(!blockState){//don't play hit sound if blocking attack
				if (Time.time > gotHitTimer && painBig && painLittle) {
					...
					if (hitPoints < 40.0f || damage > 30.0f) {
						...
						otherfx.PlayOneShot(otherfx.clip, 0.5f);
						gotHitTimer = Time.time + Random.Range(.5f, .75f);
					}
				}
			}
