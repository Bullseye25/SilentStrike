...
public class Footsteps : MonoBehaviour {
	...
	void Start () {
		...
		{
			MConstants.introduceThermal = false;
		}*/
...
//	}
    ...
    public Image CrossShair1;
    public Image CrossShair2;

    private bool _opened, _closed;
    private void Update()
    {
	    RaycastHit hit;
        ...
        if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, Mathf.Infinity))
        {
            ...
            if (objectHit.gameObject.layer == 15 )
            {
	            //if (HudMenuManager.instance.machineGunCrosshair.activeSelf && !_closed && !objectHit.gameObject.GetComponentInParent<EnemyAI>().killed && HudMenuManager.instance.machineGunCrosshair)
	            //{
		           // _opened = false;
		           // _closed = true;
		           // HudMenuManager.instance.machineGunCrosshair.GetComponent<Animator>().SetBool("close",true);
	            //}
                // CrossShair1.DOColor(Color.red, 0.3f);
                // CrossShair2.DOColor(Color.red, 0.3f);
            }
            else
            {
	            //if (HudMenuManager.instance.machineGunCrosshair.activeSelf && !_opened && HudMenuManager.instance.machineGunCrosshair)
	            //{
		           // _opened = true;
		           // _closed = false;
		           // HudMenuManager.instance.machineGunCrosshair.GetComponent<Animator>().SetBool("close",false);
	            //}
                // CrossShair1.DOColor(Color.white, 0.3f);
                // CrossShair2.DOColor(Color.white, 0.3f);
