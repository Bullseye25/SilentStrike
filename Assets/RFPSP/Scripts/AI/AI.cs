...
//			dirPerpGizmo.Normalize();
	...
	void CanSeeTarget(){
		...
		if((TargetAIComponent && !TargetAIComponent.enabled) || (target && !target.gameObject.activeInHierarchy) ||  MConstants.isGameOver)
        {
			target = null;
	...
	IEnumerator Shoot(){
		...
		if(MConstants.CurrentGameMode == MConstants.GAME_MODES.ENDLESS_MODE){
			SpawnPoint spawnPoint = EndlessModeManager.Instance.GetSpawnPointForEnemy(gameObject);
			if(spawnPoint.isSideHide){
				attackFinished = true;
				speedAmt = runSpeed;
				TravelToPoint(waypointGroup.wayPoints[waypointGroup.wayPoints.Count - 2].position);
			}
			AnimatorComponent.SetInteger("AnimState", 4);
		...
		if(MConstants.CurrentGameMode == MConstants.GAME_MODES.ENDLESS_MODE){
			...
			if(spawnPoint.isSideHide){
				//speedAmt = runSpeed;
				TravelToPoint(waypointGroup.wayPoints[waypointGroup.wayPoints.Count - 1].position);
				// attackFinished = false;
			}
			// else{
			// 	AnimatorComponent.SetInteger("AnimState", 0);
