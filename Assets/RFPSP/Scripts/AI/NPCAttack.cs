...
//	public float reloadTime = 1.75f;
    ...
    public GameObject[] DropableGuns;
    public AudioClip[] DropableGunsSounds;
    ...
    private void Awake()
    {
       ...
       int Guns = Random.Range(0, DropableGuns.Length);
        if (DropableGuns.Length!=0)
        {
            firesnd = DropableGunsSounds[Guns];
            if (this.gameObject.GetComponent<CharacterDamage>())
            {
                this.gameObject.GetComponent<CharacterDamage>().gunItem = DropableGuns[Guns];
            }
        }
